## Hi there 👋
welcone to my github page. I am learning multiple topics and usually keep assignments in this profile.
- 💻I’m currently doing multiple courses on Artificial Intelligence, Data Science and Machine learning related topics.
- 👨‍💻Learning Python (mainly), R, SQL, Power BI, Tableau.
- 🛠 Have lot more to learn (guide me).
- ☕Topics: any interesting and state of the art-it is fun to deploy and see what different models do.
- ⚡️fascinated by recent advancements and love to know more.
- 🔭Aiming to work hands on models, research papers and some practical problems.
- 💬On/Off but restarting learning with new motivation to combine EEE and CSE core works.
- ⬇️Some projects done are in repo (don't judge-as they're mostly based on coursera guidelines and requirements).
- 🍕Few domain specific projects are currently being done starting with mix of Wind prediction with different ML models and will be uploaded with some reports.
- 📜 Portfolio _Electrical and Computer based engineer-trying to update best of the both worlds.
- ⏳Strongly believe in upskilling as it saves time in the long run as multiple tools provides different advantages and easy to use specialised tool for work.
- 🎯Focussing on continuous hardwork and studying-mostly self-study and some online courses.
- <img src="https://github.com/rsmaayon/rsmaayon/assets/116261236/f9df529b-cc10-473e-9ba0-e4fa966acf21" width="25"> Always learning something because it is new or I forgot.
- 💯Cheat sheets and road maps (collected from others, mostly from LinkedIN) are also in repo, check them as it is very useful directly or indirectly.
- 💡Few other notable works (not in repo): Automation for testing, Analysis of data, Visualization with Python, R, Plotly, problem identification with pattern recognition.
- ❄️ Old school programmer with C,C++,Fortran (no, not just Hello World!) experience and programmed with Matlab, C2000, Arduino, PLC, FPGA, Raspberry Pi. Restarting the C2000 programming with basic MCUs.
- 🧩Working with Optimization(meta-heuristic) algorithms ranging from GA, PSO, GWO, ACO, ABC, MBHS, TLBO,DE, SA, CS, MS, ES,.... (and some hybrid, modifications) in matlab and python, Neural Networks (matlab toolbox and program, Python packages), Controllers (PI, Hysteresis, Deadbeat, MPC, SMC), Grid codes and standards, Machine learning methodologies, Data analysis and more programming. 
- 📍Will update the repos time to time with good works.


🌟Thank you and have a nice day 🌟.

p.s: Github is fun. Sharing and improvising is better.
<!--
**rsmaayon/rsmaayon** is a ✨ _special_ ✨ repository because its `README.md` (this file) appears on your GitHub profile.

Here are some ideas to get you started:

- 🔭 I’m currently working on ...
- 🌱 I’m currently learning ...
- 👯 I’m looking to collaborate on ...
- 🤔 I’m looking for help with ...
- 💬 Ask me about ...
- 📫 How to reach me: ...
- 😄 Pronouns: ...
- ⚡ Fun fact: ...
-->
